...
import type { MemberWithHistory, TickerEvent, Task, Priority } from '@/types'
import { MOCK_MEMBERS, MOCK_EVENTS, MOCK_TASKS } from '@/lib/mock-data'
...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    if (IS_MOCK) return
    try {
      const { getMembers, getTickerEvents, getTasks } = await import('@/lib/supabase')
      const [m, e, t] = await Promise.all([getMembers(), getTickerEvents(), getTasks()])
      setMembers(m)
      setEvents(e)
      setTasks(t)
    } catch (err) {
  ...
  async function handleAssign(
    ...
    if (IS_MOCK) {
      ...
      const { PRICE_IMPACT } = await import('@/types')
      const impact = parseFloat(
        (PRICE_IMPACT.priority[priority] * PRICE_IMPACT.demand_multiplier(member.open_tasks)).toFixed(2)
      )
          ...
          .map((m) =>
            m.id === memberId
              ? {
                  ...
                  open_tasks: m.open_tasks + 1,
                  price_change_24h: m.price_change_24h + impact,
                  price_change_pct: ((m.price_change_24h + impact) / m.base_price) * 100,
                  price_history: [
                    ...m.price_history,
                    {
                      id: `h-${Date.now()}`,
    ...
    await assignTask(title, description || null, memberId, priority)
    await loadData()
  }
          ...
          .map((m) =>
            m.id === member.id
              ? {
                  ...m,
                  current_price: newPrice,
                  open_tasks: Math.max(0, m.open_tasks - 1),
                  completed_tasks: m.completed_tasks + 1,
                  streak: m.streak + 1,
                  price_change_24h: m.price_change_24h + bonus,
                }
              : m
          )
          .sort((a, b) => b.current_price - a.current_price)
  ...
  function openAssignForMember(memberId: string) {
    setAssignDefaultId(memberId)
    setShowAssign(true)
  }
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      {selectedMember && (
        <MemberDetailModal
          ...
          onAssign={() => openAssignForMember(selectedMember.id)}
        />
      )}
    </div>
  )
